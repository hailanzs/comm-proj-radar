...
def producer_real_time_1843(q, index):
    ...
    unwrapped_phase = np.zeros(shape=(500,512), dtype=np.complex128)
    freq_spec = 0
    dca = DCA1000()
    ...
    while True:
        adc_data = dca.read()
        org_data = dca.organize(raw_frame=adc_data, num_chirps=3,
         num_rx=4, num_samples=512, num_frames=1, model='1843')
        frm_ctr+=1
        x = 0
